...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        while (true){
            ...
            System.out.println("3.Вернути квиток у касу");
    ...
    private static void reserveATicket() {
        ...
        for (int i = 0; i < townList.size(); i++){
            if (option - 1 == i){
                ...
                List<String> placeOfArrval = new ArrayList<String>();
                ...
                for (int j = 0; j < randomSize; j++){
                    ...
                    "Ціна: " +randomPrice);
                    System.out.println("----");
                    placeOfArrval.add(townList.get(randomTown));
                ...
                for (int j = 0; j < randomSize; j++){
                    if (search - 1 == j&& money >= prices.get(j)){
                        tickets.add(
                                new Ticket().setCode(random.nextInt(999999999)).addNameYourTown(townList.get(i)).addPlaceOfArrival(placeOfArrval.get(j)).addTime(times.get(j)).setPrice(prices.get(j))
                        );
                        money -= prices.get(j);
                        ...
                        System.out.println("Нажаль у вас не достатньо коштів");
                    }
                }
            }
        }
    ...
    private static void backMyTickets(){
        ...
        System.out.println("Який білет ви хочете здати?");
